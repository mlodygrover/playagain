...
// --- 1. GŁÓWNA LOGIKA (KOMPONENT WEWNĘTRZNY) ---
function AdminPrebuiltsContent() {
  ...
  const [formData, setFormData] = useState({
    ...
    image: "",
    category: "Gaming",
    components: [] as string[] // Tablica ID wybranych komponentów
  });

  // Stan pomocniczy do wyboru części w formularzu (mapa typ -> id)
  ...
  const fetchComponents = async () => {
    try {
      const res = await fetch(`${API_URL}/api/components?limit=1000`); // Pobierz dużo, żeby mieć wybór
      const data = await res.json();
  ...
  const handleOpenAdd = () => {
    setEditingId(null);
    setFormData({ name: "", description: "", price: 0, image: "", category: "Gaming", components: [] });
    setSelectedSlots({});
    setIsModalOpen(true);
  };
  ...
  const handleOpenEdit = (item: any) => {
    ...
    // Odtwórz wybrane sloty na podstawie tablicy komponentów
    const slotsMap: Record<string, string> = {};
    if (item.components && Array.isArray(item.components)) {
      item.components.forEach((comp: any) => {
        // Znajdź slot pasujący do typu komponentu
  ...
  const handleSlotChange = (type: string, componentId: string) => {
    ...
    if (!componentId) delete newSlots[type]; // Usuń jeśli wybrano "Brak"

    setSelectedSlots(newSlots);

    // Aktualizuj listę ID komponentów w formData
    setFormData(prev => ({
      ...prev,
  ...
  // Automatyczna kalkulacja ceny na podstawie wybranych części
  // Zmodernizowana funkcja kalkulacji ceny
  const calculateAutoPrice = async () => {
    // Pobierz ID wybranych komponentów (bez pustych wartości)
    const ids = Object.values(selectedSlots).filter(Boolean);

    if (ids.length === 0) {
      return alert("Wybierz najpierw komponenty!");
    }

    try {
      // Pokażmy użytkownikowi, że coś się dzieje (opcjonalnie można dodać stan loading)
      ...
      // Aktualizacja stanu i zaznaczenie pola
      setFormData(prev => ({ ...prev, price: data.totalPrice }));

      // UX: Fokus na polu ceny, żeby admin widział zmianę
      const priceInput = document.getElementById('price-input');
      if (priceInput) {
        priceInput.focus();
        // Opcjonalnie: zaznacz tekst, żeby łatwo było wpisać własną marżę
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const url = editingId
      ? `${API_URL}/api/prebuilts/${editingId}`
      : `${API_URL}/api/prebuilts`;
    ...
    <div className="min-h-screen bg-black text-white pt-32 px-4 pb-20">
      <div className="max-w-6xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          ...
          <button onClick={handleOpenAdd} className="bg-blue-600 hover:bg-blue-500 px-5 py-2.5 rounded font-bold uppercase flex items-center gap-2 text-sm transition-all">
            ...
            <div className="bg-zinc-900 border border-zinc-700 w-full max-w-4xl max-h-[90vh] overflow-y-auto rounded-xl shadow-2xl p-6">
              ...
              <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <div className="relative">
                        <input
                          id="price-input" // Dodane ID dla focusowania
                          type="number"
                          required
                          value={formData.price}
                          onChange={e => setFormData({ ...formData, price: Number(e.target.value) })}
                          className="w-full bg-black border border-zinc-700 p-3 rounded text-sm text-white font-mono text-green-400 focus:border-green-500 focus:ring-1 focus:ring-green-500 outline-none transition-all"
                        />
                        <button
                          id="calc-btn" // Dodane ID dla animacji
                          type="button"
                          onClick={calculateAutoPrice}
                          title="Oblicz automatycznie z bazy danych"
                      ...
                      <p className="text-[10px] text-zinc-600 mt-1 ml-1">Kliknij kalkulator, aby pobrać sumę części.</p>
                    </div>
                  </div>
                ...
                <div className="space-y-3 bg-black/30 p-4 rounded-lg border border-zinc-800">
                  ...
                  {SLOTS.map(slot => {
                    // Filtrujemy komponenty pasujące do typu slotu
...
// --- 2. GŁÓWNY WRAPPER (EKSPORTOWANY) ---
// Suspense naprawia błąd "useSearchParams" podczas budowania na Netlify
