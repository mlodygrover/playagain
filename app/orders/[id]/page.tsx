...
import {
    ...
    ChevronRight, AlertCircle, Printer, Truck // Dodano Truck
} from "lucide-react";
...
export default function OrderDetailsPage() {
    ...
    useEffect(() => {
        // Pozwalamy na wjazd bez tokena (Smart Auth na backendzie obsłuży resztę)
        const fetchOrderAndReturn = async () => {
            try {
                ...
                const orderRes = await fetch(`${API_URL}/api/orders/${id}`, { headers });

                if (!orderRes.ok) {
                    if (orderRes.status === 401 || orderRes.status === 403) {
                        // Zapamiętujemy aktualną ścieżkę (np. /zamowienie/6977...)
                ...
                // 2. Pobierz dane zwrotu
                if (orderData.status === "PAID" || orderData.status === "SHIPPED") {
                    const returnRes = await fetch(`${API_URL}/api/returns/by-order/${id}`, { headers });
                    if (returnRes.ok) {
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        ...
                        <div className="bg-zinc-900/30 border border-zinc-800 p-8 relative overflow-hidden">
                            <div className={`absolute top-0 left-0 w-1 h-full ${order.status === 'PAID' || order.status === 'SHIPPED' ? 'bg-green-500' : 'bg-yellow-500'}`} />
                            <div className="flex justify-between items-start mb-6">
                                <div>
                                    <span className="text-[10px] text-zinc-500 uppercase font-black tracking-widest block mb-1">Status zamówienia</span>
                                    <h2 className={`text-2xl font-black uppercase tracking-tight ${order.status === 'PAID' || order.status === 'SHIPPED' ? 'text-green-400' : 'text-yellow-500'}`}>
                                        {order.status === 'PAID' && 'Opłacone'}
                                        {order.status === 'SHIPPED' && 'Wysłane'}
                                        {order.status === 'PENDING' && 'Oczekuje na płatność'}
                            ...
                            {/* --- NOWOŚĆ: KAFELEK TRACKINGU --- */}
                            {order.trackingLink && (
                                <div className="mb-6 bg-gradient-to-br from-green-500/20 to-emerald-900/10 border border-green-500/30 p-6 rounded-lg flex flex-col sm:flex-row items-center justify-between gap-6 relative overflow-hidden group">
                                    <Truck className="absolute -right-4 -bottom-4 w-24 h-24 text-green-500/5 -rotate-12 group-hover:scale-110 transition-transform duration-700" />

                                    <div className="flex items-center gap-5 relative z-10">
                                        <div className="bg-green-500 p-3 rounded-full shadow-[0_0_20px_rgba(34,197,94,0.4)] animate-pulse">
                                            <Truck className="w-6 h-6 text-black" />
                                        </div>
                                        <div>
                                            <p className="text-white font-black uppercase tracking-tighter text-lg leading-none mb-1">Paczka w drodze!</p>
                                            <p className="text-green-200/60 text-[10px] font-bold uppercase tracking-widest">Twoje zamówienie zostało przekazane kurierowi</p>
                                        </div>
                                    </div>
                                    ...
                                    <a
                                        ...
                                        className="bg-white hover:bg-green-400 text-black font-black px-8 py-3 uppercase text-[10px] tracking-[0.2em] transition-all relative z-10 shadow-xl flex items-center gap-2 group/btn shrink-0"
                                    >
                                        Śledź przesyłkę
                            ...
                            {order.status === 'PENDING' && (
                                <div className="bg-yellow-500/10 border border-yellow-500/20 p-6 rounded-lg flex flex-col sm:flex-row items-center justify-between gap-4">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <p className="text-sm text-yellow-200">Zamówienie nie zostało jeszcze opłacone.</p>
                                    </div>
                                    <button
                            ...
                            {/* AKCJA: ZWROT (jeśli PAID/SHIPPED) */}
                            {(order.status === 'PAID' || order.status === 'SHIPPED') && returnInfo && (
                                <div className="bg-zinc-800/50 border border-zinc-700 p-6 rounded-lg flex flex-col sm:flex-row items-center justify-between gap-4">
                                    <div className="flex items-center gap-4">
                                        <RotateCcw className={`w-8 h-8 ${returnInfo.status !== 'NONE' ? 'text-blue-500' : 'text-zinc-500'}`} />
                                        <div>
                                            <p className="text-sm font-bold text-white uppercase tracking-tight">Odstąpienie od umowy / RMA</p>
                                            <p className="text-[11px] text-zinc-400 font-mono">Masz 14 dni na zgłoszenie zwrotu lub usterki.</p>
                                        </div>
                                    </div>
                                    <Link
                                        href={`/returns/${order._id}`}
                                        className="bg-zinc-700 hover:bg-zinc-600 text-white font-bold px-6 py-3 uppercase text-[10px] tracking-widest transition-all w-full sm:w-auto text-center"
                                    >
                                        {returnInfo.status === 'NONE' ? 'Zgłoś zwrot' : 'Sprawdź status'}
                                    </Link>
                                </div>
                            )}
                        </div>
                    ...
                    <div className="space-y-6">
                        <div className="bg-zinc-950 border border-zinc-800 p-6">
                            ...
                            <div className="text-sm text-zinc-300 space-y-1">
                                ...
                                <p className="pt-2 text-zinc-500 font-mono text-[11px]">{order.customerDetails?.email}</p>
                            </div>
                        </div>
                        ...
                        <div className="bg-zinc-950 border border-zinc-800 p-6">
                            ...
                            <div className="space-y-3 mb-6 font-mono text-[11px]">
                                <div className="flex justify-between">
                                    <span className="text-zinc-500">Kwota netto</span>
                                    <span className="text-zinc-300">{(order.totalAmount * 0.77).toFixed(2)} PLN</span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="text-zinc-500">Podatek (0% / MARŻA)</span>
                                    <span className="text-zinc-300">0.00 PLN</span>
                                </div>
                            </div>
                            <div className="border-t border-zinc-800 pt-4 flex justify-between items-end">
                                <span className="font-black uppercase text-[10px] tracking-widest text-zinc-500">Razem</span>
