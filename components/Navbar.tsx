        ...
        <div className="max-w-[1600px] mx-auto px-4 md:px-6 h-full flex items-center justify-between">
          ...
          >
            <img
              ...
              className="w-48 xl:w-60 h-16 object-contain" // Zmniejszone logo na mniejszych ekranach
            />
          </Link>

          {/* DESKTOP MENU - Zmieniono z md:flex na xl:flex (widoczne od 1280px) */}
          ...
          <div className="flex items-center gap-4 z-50">
            ...
            {/* ADMIN (DESKTOP) - Widoczny tylko powyżej xl */}
            {isAdmin && (
              <Link
                href="/admin"
                title="Panel Administratora PlayAgain"
                className="hidden xl:flex items-center gap-2 text-xs font-bold text-red-500 hover:text-red-400 uppercase tracking-widest transition-colors mr-4 border border-red-900/30 bg-red-900/10 px-3 py-1.5 rounded"
              >
                <ShieldAlert className="w-4 h-4" /> Admin
              </Link>
            )}

            {/* LOGOWANIE / KONTO (Desktop) - Zmieniono md na xl */}
            {user ? (
              <div className="hidden xl:flex items-center gap-3 mr-2 border-r border-zinc-800 pr-4">
                <Link
                  href="/profil"
              ...
              <Link
                ...
                className="hidden xl:flex items-center gap-2 text-xs font-bold text-zinc-400 hover:text-white uppercase tracking-widest transition-colors mr-2 border-r border-zinc-800 pr-4"
              >
            ...
            {/* CTA - Zmieniono z sm na xl */}
            <Link
              href="/konfigurator"
              title="Rozpocznij konfigurację komputera w PlayAgain"
              className="hidden xl:block bg-white text-black text-xs font-bold px-5 py-2 uppercase tracking-widest hover:bg-blue-600 hover:text-white transition-all skew-x-[-10deg]"
            >
              <span className="skew-x-[10deg] inline-block text-center">Zbuduj PC</span>
            </Link>

            {/* MOBILE HAMBURGER - Widoczny poniżej xl (1280px) */}
            <button
              className="xl:hidden text-white p-2 hover:bg-zinc-800 rounded transition-colors"
      ...
      {isMobileMenuOpen && (
        <div className="fixed top-16 left-0 w-full h-[calc(100vh-64px)] bg-black z-40 flex flex-col p-6 overflow-y-auto animate-in slide-in-from-top-5 duration-300">
            ...
            {user ? (
              <div className="flex items-center justify-between">
                <Link href="/profil" onClick={closeMenu} className="group" title="Twój profil PlayAgain">
                  <p className="text-xs text-zinc-500 uppercase group-hover:text-blue-500 transition-colors leading-none">Witaj</p>
              ...
              <Link href="/login" onClick={closeMenu} className="flex items-center justify-center gap-2 text-blue-500 font-bold uppercase py-4 border border-blue-500/20 rounded" title="Zaloguj się do PlayAgain">
          ...
          <div className="flex flex-col gap-2">
            ...
            <MobileLink href="/o-nas" onClick={closeMenu}>O Nas</MobileLink> {/* Dodano O Nas do mobile */}
          </div>
...
const NavLink = ({ href, children, title }: { href: string; children: React.ReactNode; title?: string }) => (
  <Link
    href={href}
    title={title}
    className="text-xs font-mono text-zinc-400 hover:text-blue-500 uppercase tracking-widest transition-colors relative group flex items-center justify-center text-center h-full min-w-fit"
  >
    {children}
